...
export function createWindmill(scene: THREE.Scene) {
    ...
    // 右奥の小島（5, 1.5, -7）のスケール0.5 に配置
    // ワールド座標で島の上に
    group.position.set(5, 2.0, -7)
    ...
    // 塔（円柱、少し上に細くなる）
    const towerGeo = new THREE.CylinderGeometry(0.2, 0.35, 1.2, 6)
    ...
    // 屋根（円錐）
    const roofGeo = new THREE.ConeGeometry(0.3, 0.3, 6)
    ...
    bladesGroup.position.set(0, 1.0, 0.35)
    ...
    // 中心軸
    const axleGeo = new THREE.CylinderGeometry(0.03, 0.03, 0.1, 4)
